...
function cloudIamContract(seed:number,index:number):GeneratedContract{const random=mulberry32(seed);const role=pick(random,['report-runner','archive-reader','build-export']);const bucket=pick(random,['ledger-archive','case-exports','route-backups']);return{id:`cloud-iam-${seed}`,seed,type:'CLOUD_IAM_REVIEW',title:'Облачная роль с лишними правами',client:'Greylock / cloud desk',factionId:'line',factionName:'Greylock',skill:'cloud',difficulty:index%2===0?'STANDARD':'HARD',pay:18000+index*500,summary:`Роль ${role} читала ${bucket} и имеет неиспользуемые KMS/IAM действия.`,constraint:'Сузь policy по actions/resources и не ломай рабочий export.',files:[{name:'policy.json',content:`role=${role}\naction=*\nresource=*\nbucket=${bucket}`},{name:'trail.log',content:`GetObject ${bucket}/daily.csv principal=${role}\nCreateGrant kms/archive principal=${role}`}],questions:[{id:'role',label:'Какая роль требует исправления',placeholder:'role',answers:[role]},{id:'bucket',label:'Какой bucket затронут',placeholder:'bucket',answers:[bucket]}],hint:'Проверь data events, last accessed и KMS grants. Переведи workload на temporary role.'};}
function containerContract(seed:number,index:number):GeneratedContract{const random=mulberry32(seed);const namespace=pick(random,['payments','archive','dispatch']);const sa=pick(random,['api-runner','exporter','sync-agent']);return{id:`container-${seed}`,seed,type:'CONTAINER_SUPPLY_REVIEW',title:'Неподписанный образ и лишний RBAC',client:'Greylock / platform',factionId:'line',factionName:'Greylock',skill:'container',difficulty:'HARD',pay:21000+index*500,summary:`В namespace ${namespace} запущен mutable image, ServiceAccount ${sa} имеет cluster-wide access.`,constraint:'Сохрани audit и artifact metadata до остановки bad digest.',files:[{name:'pods.txt',content:`namespace=${namespace}\nimage=registry.local/api:stable\nserviceAccount=${sa}`},{name:'rbac.txt',content:`${sa}=cluster-admin`}],questions:[{id:'namespace',label:'Namespace',placeholder:'namespace',answers:[namespace]},{id:'sa',label:'ServiceAccount',placeholder:'name',answers:[sa]}],hint:'Проверь digest/signature, effective RBAC, audit log и egress policy.'};}
function architectureContract(seed:number,index:number):GeneratedContract{const random=mulberry32(seed);const system=pick(random,['settlement','warehouse','identity']);const rto=pick(random,['30m','45m','60m']);const rpo=pick(random,['5m','10m','15m']);return{id:`architecture-${seed}`,seed,type:'ARCHITECTURE_REVIEW',title:'Проект восстановления и Zero Trust',client:'Greylock / engineering',factionId:'line',factionName:'Greylock',skill:'architecture',difficulty:'HARD',pay:23000+index*500,summary:`Для ${system} нужны access model и recovery design с RTO ${rto}, RPO ${rpo}.`,constraint:'Не считай внутреннюю сеть доверенной и проверь restore test.',files:[{name:'requirements.txt',content:`system=${system}\nRTO=${rto}\nRPO=${rpo}\ncritical=true`},{name:'current.txt',content:'trust=inside-network\nbackup=online-only\nlogs=endpoint-only'}],questions:[{id:'system',label:'Критичная система',placeholder:'system',answers:[system]},{id:'rto',label:'RTO',placeholder:'time',answers:[rto]},{id:'rpo',label:'RPO',placeholder:'time',answers:[rpo]}],hint:'Identity-aware access, segmentation, immutable backup, restore test, telemetry and risk owner.'};}

const builders = [authContract, dnsContract, networkSegmentContract, processContract, linuxPersistenceContract, windowsContract, pythonContract, secretContract, webContract, timelineContract, apiAuthorizationContract, sqlReviewContract, mobileProfileContract, mobileTokenContract, adStaleAccountContract, adGpoAclContract, mailHeaderContract, mailOauthContract, forensicTimelineContract, forensicDeletedArtifactContract, irContainmentContract, irRecoveryContract, huntRareAccountContract, huntTelemetryGapContract, certificateReviewContract, malwareStaticContract, cloudIamContract, containerContract, architectureContract] as const;
...
export function generateContractOffers(progress: ProgressState, refreshIndex = 0): GeneratedContract[] {
  ...
  const candidates = builders
    ...
    .filter((contract) => progress.cloudCaseComplete || contract.type !== 'CLOUD_IAM_REVIEW')
    .filter((contract) => progress.supplyCaseComplete || contract.type !== 'CONTAINER_SUPPLY_REVIEW')
    .filter((contract) => progress.architectureCaseComplete || contract.type !== 'ARCHITECTURE_REVIEW');
